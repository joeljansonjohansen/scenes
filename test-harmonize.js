...
function setupModules() {
	...
	let playerModuleThree = new GrainPlayerModule({
		start: "5m",
		length: "10m",
		transpose: 0,
		loopLength: "2m",
	...
	let playerModuleFive = new GrainPlayerModule({
		start: "25m",
		length: "20m",
		transpose: -5,
		loopLength: "2m",
		...
		onEnd: () => {
			console.log("module finished");
			modules.splice(modules.indexOf(playerModuleFive), 1);
		},
	});
	let playerModuleSix = new GrainPlayerModule({
		start: "25m",
		length: "20m",
		transpose: -1,
		loopLength: "2m",
		...
		onEnd: () => {
			console.log("module finished");
			modules.splice(modules.indexOf(playerModuleSix), 1);
		},
	});
	...
	let recorderModuleTwo = new RecorderModule({
		title: "Recording",
		start: "20m",
		length: "5m",
		mic: mic,
		onEnd: () => {
			playerModule.prepareModule({
				recordingURL: recorderModuleTwo.recordingURL,
				moduleReady: () => {
					// const fShift = new Tone.PitchShift(-0.5).toDestination();
					// playerModuleTwo.connect(fShift);
					//playerModule.connect(reverb);
			...
			/* playerModuleFive.prepareModule({
				recordingURL: recorderModuleTwo.recordingURL,
				moduleReady: () => {
					console.log("Recorder module connected to other");
					playerModuleFive.connect(reverb);
					modules.push(playerModuleFive);
				},
			});
			playerModuleSix.prepareModule({
				recordingURL: recorderModuleTwo.recordingURL,
				moduleReady: () => {
					console.log("Recorder module connected to");
					playerModuleSix.connect(reverb);
					modules.push(playerModuleSix);
				},
			}); */
			modules.splice(modules.indexOf(recorderModuleTwo), 1);
		},
	});
	modules.push(recorderModuleTwo);
}
